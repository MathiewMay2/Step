package ca.cobiy.step.Libs.v1_14;
...
public class NPC_1_14_R1 implements NPC {
	...
	public void crouchNPC(String id) {
		if(npcList.containsKey(id)) {
			...
			dataWatcher.set(DataWatcherRegistry.s.a(6), EntityPose.SNEAKING);
			for(Player player : Bukkit.getOnlinePlayers()) {
				((CraftPlayer) player).getHandle().playerConnection.sendPacket(new PacketPlayOutEntityMetadata(npc.getId(), dataWatcher, false));
			}
			updateNPC(id);
		}else {
			plugin.Log("&4Could not execute crouchNPC on '"+id+"' as it was not found.");
